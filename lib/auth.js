...
exports.sessionStatus = function(req, callback) {
  ...
  db().get(SQL_GET_SESSION, req.headers['x-api-key'], function(err, keyObj) {
    if (err) {
    ...
    if (keyObj.expire && keyObj.expire < Date.now()) {
      ...
      db().run(SQL_DELETE_SESSION, req.headers['x-api-key'], function(err, data) {
...
exports.verifyAssetExists = function(req, res, next) {
  db().get(SQL_GET_ASSET, req.params.assetName, function(err, data) {
    if (err || !data) {
